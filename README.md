...
[![Git LFS](https://img.shields.io/badge/Git-LFS-orange?logo=git&logoColor=white)](https://git-lfs.github.com/)
...
Model weights (`.pth`, `.onnx`, `.onnx.data`, and `.pt`) are tracked and managed using **Git LFS (Large File Storage)**, keeping the repository history clean.
...
```
.
├── cv-pipeline/           # Main computer vision project directory
│   ├── api/               # FastAPI REST web server
│   │   └── main.py
│   ├── data/              # Auto-downloaded dataset files (gitignored)
│   ├── models/            # Saved model checkpoints and ONNX files (tracked via Git LFS)
│   │   ├── class_names.json     # Generated 37 breed class name mapping
│   │   ├── training_log.csv     # Metric progress logs for classification
│   │   ├── resnet50_pets_best.pth  # PyTorch weights checkpoint (LFS)
│   │   └── resnet50_pets.onnx      # Exported ONNX model graph (LFS)
│   ├── src/               # Processing & utility scripts
...
│   ├── yolov5s.pt         # YOLOv5 pretrained weights (LFS)
│   └── .gitignore         # Sub-directory Gitignore
...
Navigate to the `cv-pipeline` directory and create/activate a Python virtual environment:
...
*(Note: Run these commands inside the `cv-pipeline/` directory with the virtual environment activated)*
